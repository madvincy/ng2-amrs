...
export class HeiIndicatorsFilterComponent
  implements OnInit {
  ...
  public onClickedGenerate() {
      this.setQueryParams();
      // this.selectedFilter.emit(this.params);
      // console.log('set Params', this.params);
  ...
  public setQueryParams() {
    this.params = {
        ...
        'startDate' : Moment(this.monthString, 'YYYY-MM').startOf('month').format('YYYY-MM-DD'),
        'endDate': Moment(this.monthString, 'YYYY-MM').endOf('month').format('YYYY-MM-DD'),
    };
