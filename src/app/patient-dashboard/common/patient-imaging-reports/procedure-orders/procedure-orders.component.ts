...
export class ProcedureOrdersComponent implements OnInit, OnDestroy {
  ...
  public getProcedureOrders(): Observable<any[]> {
    ...
    const procedureOrderUuid = '52a447d3-a64a-11e3-9aeb-50e549534c5e';
  ...
  public assignValue(data): any {
    ...
    for (let b = 0; b < value.length; b++) {
      if (value) {
        ...
        } else if (value[b].concept.uuid === data.order.concept.uuid) {
          resultsData.findings = value[b].value;
        } else {
          resultsData.comments.push(value[b]);
        }
      }
    }
    return resultsData;
  ...
  public showResults(order): void {
    ...
    this.imagingComments = order.status.comments;
  ...
  public saveProcedureOrder(procedureOrderPayload): void {
    const encounterPayload = {
      patient: this.patient.uuid,
      encounterType: '125d8a4e-6154-4cbd-bd03-8fb1b0f7ed5c',
  ...
  public sendResults(): void {
    if (this.imageReportFindings) {
      this.dataPayload.findings = this.imageReportFindings;
    }
    if (this.imageReportFindingsText) {
      this.dataPayload.notes = this.imageReportFindingsText;
    }
  ...
  public results(value) {
    // console.log(value);
    if (value.uuid === '5622AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA') {
  ...
  public determineObsPayload(data): void {
    // console.log(this.orderResultsConcept.results, 'data');
    this.obsPayload = {
      obsDatetime: this.toDateString(new Date()),
      // encounterType: '5021b1a1-e7f6-44b4-ba02-da2f2bcf8718',
      location: this.locationUuid,
      concept: this.orderResultsConcept.results,
    ...
    if (data.reportType === this.orderResultsConcept.report) {
      ...
      if (data.findings) {
        this.obsPayload.groupMembers.push( {
          concept: this.selectedProcedureTests.uuid,
          value: data.findings.uuid,
          obsDatetime: this.toDateString(new Date()),
          person: this.patient.uuid
        }
        );
      ...
      this.saveEncounterObs(this.obsPayload);
    }
  ...
  public saveEncounterObs(obsPayload): any {
    let response: any;
